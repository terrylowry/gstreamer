...
static gboolean
dump_structure_field (GQuark field_id, const GValue * value,
    gpointer user_data)
{
  auto str = (GString *) user_data;
  const char *fieldname = g_quark_to_string (field_id);
  gchar *val;
...
gst_wasapi2_dump_devices (GList * device_list)
{
  ...
  for (iter = device_list; iter; iter = g_list_next (iter)) {
    ...
    g_string_append_printf (str, "%s (%s)\n", name, device_class);
    gst_structure_foreach (prop, dump_structure_field, str);
