...
rtp_source_class_init (RTPSourceClass * klass)
{
   ...
   * receiver reports.
   *
   * * "received-rr"  GST_TYPE_ARRAY   Array of GstStructure entries, one for each peer that is sending RR.
   *
...
_create_rr_entry (gpointer * key, gpointer val, gpointer userdata)
{
  ...
  gst_value_array_append_and_take_value (res, &v);
...
{
  GValue rr_array = G_VALUE_INIT;
  g_value_init (&rr_array, GST_TYPE_ARRAY);
  g_hash_table_foreach (src->received_rr, (GHFunc) _create_rr_entry, &rr_array);

  gst_structure_take_value (s, "received-rr", &rr_array);
}
    ...
    guint32 exthighestseq, guint32 jitter, guint32 lsr, guint32 dlsr)
{
  ...
  if (src->internal) {
     ...
     * internal sources to track which remote sources sent reports for this */
